import Link from "next/link";
import Image from "next/image";
import { TrendItem } from "@/lib/google-trends";
import { getCountryByCode } from "@/data/countries";

interface TrendCardProps {
  trend: TrendItem;
  rank: number;
}

export default function TrendCard({ trend, rank }: TrendCardProps) {
  const country = getCountryByCode(trend.country);

  return (
    <article>
      <Link
        href={`/trend/${trend.slug}`}
        className="group block rounded-2xl border border-gray-200 bg-white overflow-hidden shadow-sm transition-all hover:shadow-xl hover:border-gray-300 hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
        aria-label={`#${rank} Trending: ${trend.title} in ${country?.name} - ${trend.traffic} searches`}
      >
        <div className="flex flex-col sm:flex-row">
          {/* Left: Rank + Image */}
          <div className="relative sm:w-48 shrink-0">
            {trend.imageUrl ? (
              <div className="relative h-40 sm:h-full w-full overflow-hidden">
                <Image
                  src={trend.imageUrl}
                  alt={`${trend.title} trending topic in ${country?.name}`}
                  fill
                  sizes="(max-width: 640px) 100vw, (max-width: 768px) 192px, 192px"
                  className="object-cover transition-transform duration-500 group-hover:scale-105"
                  loading="lazy"
                  quality={75}
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" />
              </div>
            ) : (
              <div
                className="h-40 sm:h-full w-full flex items-center justify-center"
                style={{ backgroundColor: `${country?.color}15` }}
                aria-hidden="true"
              >
                <span className="text-5xl opacity-30">{country?.flag}</span>
              </div>
            )}
            <div
              className="absolute top-3 left-3 w-10 h-10 rounded-full flex items-center justify-center text-lg font-black text-white shadow-lg border-2 border-white"
              style={{ backgroundColor: country?.color || "#3B82F6" }}
              aria-label={`Rank ${rank}`}
            >
              {rank}
            </div>
          </div>

          {/* Right: Content */}
          <div className="flex-1 p-5">
            {/* Meta */}
            <div className="flex items-center gap-2 mb-2">
              <span className="text-base" aria-hidden="true">{country?.flag}</span>
              <span className="text-xs text-gray-500 font-medium">
                {country?.name}
              </span>
              <span className="text-xs text-gray-300" aria-hidden="true">|</span>
              <span
                className="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-bold text-white"
                style={{ backgroundColor: country?.color || "#3B82F6" }}
                aria-label={`${trend.traffic} searches`}
              >
                {trend.traffic}
              </span>
              {trend.date && (
                <>
                  <span className="text-xs text-gray-300" aria-hidden="true">|</span>
                  <time
                    dateTime={trend.date}
                    className="text-xs text-gray-400"
                  >
                    {new Date(trend.date).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                  </time>
                </>
              )}
            </div>

            {/* Title */}
            <h3 className="font-black text-gray-900 group-hover:text-blue-600 transition-colors text-xl mb-2 leading-tight">
              {trend.title}
            </h3>

            {/* Summary or Description */}
            {(trend.summary || trend.description) && (
              <p className="text-sm text-gray-600 leading-relaxed line-clamp-2 mb-3">
                {trend.summary || trend.description}
              </p>
            )}

            {/* Reactions preview */}
            {trend.reactions && (
              <div className="flex items-start gap-2 bg-amber-50 rounded-lg px-3 py-2 mb-3">
                <span className="text-sm shrink-0" aria-hidden="true">💬</span>
                <p className="text-xs text-amber-800 line-clamp-1">
                  {trend.reactions}
                </p>
              </div>
            )}

            {/* Tags */}
            <div className="flex items-center justify-between">
              <div className="flex flex-wrap gap-1.5" role="list" aria-label="Related searches">
                {trend.relatedQueries.slice(0, 3).map((q) => (
                  <span
                    key={q}
                    role="listitem"
                    className="inline-block rounded-full bg-gray-100 px-2.5 py-0.5 text-xs text-gray-600 font-medium"
                  >
                    {q}
                  </span>
                ))}
              </div>
              <span className="text-xs text-blue-500 font-medium group-hover:underline ml-2 shrink-0">
                View details →
              </span>
            </div>
          </div>
        </div>
      </Link>
    </article>
  );
}
